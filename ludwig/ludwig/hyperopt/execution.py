...
class RayTuneExecutor:
    ...
    def _run_experiment(
        ...
        for gpu_id in ray.get_gpu_ids():
            ...
            # In shared GPU environments, the default target_util=0.01 is often too strict
            # and may fail trials even when training can proceed safely.
            target_util = float(os.getenv("LUDWIG_GPU_TARGET_UTIL", "0.20"))
            retry = int(os.getenv("LUDWIG_GPU_WAIT_RETRY", "20"))
            delay_s = int(os.getenv("LUDWIG_GPU_WAIT_DELAY_S", "2"))
            try:
                wait_for_gpu(gpu_id, target_util=target_util, retry=retry, delay_s=delay_s)
            except RuntimeError as e:
                logger.warning(
                    "wait_for_gpu timed out for gpu_id=%s (target_util=%.2f, retry=%s, delay_s=%s): %s. "
                    "Continue anyway to avoid trial crash.",
                    gpu_id,
                    target_util,
                    retry,
                    delay_s,
